...
\setbeamercolor*{palette secondary}{use=structure,fg=white,bg=structure.fg!60}
...
\begin{frame}[fragile,shrink=20]
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item Alcuni display aspettano di avere 1+ fotogrammi prima di visualizzarli, per \alert{applicare dell'elaborazione}
		...
		\begin{figure}
			...
			\begin{tikzpicture}
				\begin{axis}[xbar, bar width=8pt, y dir=reverse, ytick=data, yticklabels from table={\dataset}{model}, yticklabel style={text width=2.5cm, align=right}, table/y expr = \coordindex, nodes near coords, reverse legend, legend style={at={(5.25cm,-0.75cm)},anchor=east},xlabel=Ritardo (ms), width=6.5cm, height=8.25cm, xmin=0, ymin=-1, ymax=8] %ymax messo a mano con il numero di display per migliore formattazione
...
\begin{frame}[fragile,shrink=25]
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item Le \alert{GPU Intel hanno un ritardo sensibilmente maggiore}, la cui causa è probabilmente il framerate inferiore durante il test
		\end{itemize}
		\column{0.65\textwidth}
		\begin{figure}
			\pgfplotstableread[col sep=comma]{
			...
			\begin{tikzpicture}
				\begin{axis}[xbar, bar width=8pt, y dir=reverse, ytick=data, yticklabels from table={\dataset}{config}, yticklabel style={text width=2cm, align=right}, table/y expr = \coordindex, nodes near coords, reverse legend, legend style={at={(0.5,6.2cm)},anchor=south}, xlabel=Ritardo (ms), width=7cm, height=7.5cm, ymin=-1, ymax=7,xmax=125]
					\addplot table[x=vsync] {\dataset};
					\addplot table[x=novsync] {\dataset};
					\legend{VSync On, VSync Off}
				\end{axis}
...
\begin{frame}[fragile,shrink=15]
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item \alert{Ritardo di Google Stadia diminuito rispetto al lancio} (era 180-300ms), ma varianza elevata causa stuttering
		\end{itemize}
		\column{0.65\textwidth}
		\begin{figure}
			\pgfplotstableread[col sep=comma]{
			...
			\begin{tikzpicture}
				\begin{axis}[xbar, bar width=10pt, y dir=reverse, ytick=data, yticklabels from table={\dataset}{app}, yticklabel style={text width=2.5cm, align=right}, table/y expr = \coordindex, nodes near coords, xlabel=Ritardo (ms), width=6cm, height=7.5cm, ymin=-1, ymax=8] %ymax messo a mano con il numero di test per migliore formattazione
...
\begin{frame}[fragile,shrink=30]
	\frametitle{Risposta dei pixel e overdrive}
	\begin{itemize}
		\item \alert{Test automatico} tra ogni coppia di sfumature di grigio, \alert{quì mostrati media geometrica e varianza}
		\item Display recenti, TN, e \alert{"da gaming" hanno i migliori tempi di risposta}
		\item \alert{IPS moderni non sono molto più lenti} rispetto ai TN
		\item \alert{Produttori spingono l'overdrive} al massimo per ridurre i tempi di risposta a scapito della qualità dell'immagine
	\end{itemize}
	\begin{columns}
		\column{0.5\textwidth}
		\begin{figure}
			...
			\begin{tikzpicture}
				\begin{axis}[xbar, bar width=8pt, y dir=reverse, ytick=data, yticklabels from table={\dataset}{model}, yticklabel style={text width=2cm, align=right}, table/y expr = \coordindex, nodes near coords, reverse legend, xlabel=Tempo di risposta (ms), width=6.25cm, height=7.25cm, xmin=0, ymin=-1, ymax=6] %ymax messo a mano con il numero di display per migliore formattazione
		...
		\begin{figure}
			...
			\begin{tikzpicture}
				\begin{axis}[xbar, bar width=8pt, y dir=reverse, ytick=data, yticklabels from table={\dataset}{model}, yticklabel style={text width=2cm, align=right}, table/y expr = \coordindex, nodes near coords, reverse legend, xlabel=Errore di transizione (\%), width=6.25cm, height=7.25cm, xmin=0, ymin=-1, ymax=6] %ymax messo a mano con il numero di display per migliore formattazione]
					\addplot plot[forget plot] table[x=fix] {\dataset};
					\addplot plot [error bars/.cd, x dir = both, x explicit] table[x=odoff, x error plus=odoffemax, x error minus=odoffemin] {\dataset};
					\addplot plot [error bars/.cd, x dir = both, x explicit] table[x=odon, x error plus=odonemax, x error minus=odonemin] {\dataset};
					%\legend{Overdrive Off, Overdrive On}
...
\begin{frame}
	...
	\begin{itemize}
		\item PWM e noise: \begin{itemize}
			...
			\item \alert{Su alcuni display è rilevabile il ciclo di refresh dei pixel}, cosa inaspettata
		\end{itemize}
		\item Microstuttering: \begin{itemize}
			\item \alert{Nessun display ha mostrato microstuttering a refresh rate nativo}, come atteso
