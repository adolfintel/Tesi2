...
\end{figure}\todo{atrent: se le foto non sono tue devi citare la fonte}
...
Il sensore ha un tempo di risposta di circa 0.1ms in salita e 0.2ms in discesa \cite{als_pt19_datasheet}, che è stato verificato essere addirittura inferiore collegandolo ad un oscilloscopio\todo{atrent: screenshot oscilloscopio?}, quindi è sufficientemente veloce da poter misurare non solo la latenza totale del sistema, ma anche il comportamento della retroilluminazione e la risposta dei pixel, cosa che non era possibile usando un semplice fotoresistore.
...
\begin{itemize}
	\item È possibile posizionare un vetrino da microscopio 22x22mm nell'apposito incavo sul fondo della base, per progettere\todo{typo} il sensore da tocchi accidentali. Per tenere fermo il vetrino si consiglia di usare della resina ipossidica\todo{atrent: typo "epossidica", lancia uno spellchecker}, facendo attenzione a non sporcare il vetro posizionandola (figura \ref{fig:assembly_12})
...
\begin{itemize}
	\item \textbf{Script automatico}: assieme al firmware viene fornito uno script che flasha il dispositivo con un firmware precompilato e testato. Questa è la soluzione consigliata per chi vuole solo utilizzare OpenLDAT e non sviluppare. Al momento lo script è disponibile per GNU/Linux e Windows\todo{atrent: io avrei lasciato perdere windows ma ti capisco}
