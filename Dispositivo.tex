...
\begin{table}
	\centering
	\adjustbox{max width=\textwidth}{\begin{tabular}{|c|c|c|} 
		\hline
...
\begin{table}[h!]
	\centering
	\adjustbox{max width=\textwidth}{\begin{tabular}{|l|l|l|l|} 
		\hline
...
\begin{itemize}
	\item I dispositivi OpenLDAT vengono trovati in base all'ID hardware personalizzato. Il dispositivo si dichiara con il nome ``OpenLDAT Model 1'' e ID hardware (VID:PID) 4370:0001, il che lo rende facilmente distinguibile da altri dispositivi basati sulla stessa board che potrebbero essere connessi al PC. La figura \ref{fig:lsusb} mostra un output del comando \texttt{lsusb} di GNU/Linux mentre il dispositivo è connesso. Non è necessario installare alcun driver per comunicare col dispositivo
...
\begin{table}[h!]
	\centering
	\adjustbox{max width=\textwidth}{\begin{tabular}{|c|c|c|c|c|} 
		\hline
